...
- **notification-worker** → on `victim.identified`, loads the victim's `emergency_contacts` (JSONB, shape `{ name, relationship, phone, backupPhone, email }`) and emails any contact that carries an `email` via SMTP (nodemailer; auth = `SMTP_USER` + `SMTP_PASS` app password). Contacts without an email are skipped.
...
1. **Phone-only emergency contacts** — `notification-worker` alerts via **email only** (SMTP). Emergency contacts carry `phone`/`backupPhone` too, but contacts without an `email` are not notified. Adding an SMS channel is deferred; for now, ensure at least one contact has an email.
